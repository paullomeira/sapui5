  ...
  function (
    ...
    return BaseController.extend("sapui5.template.controller.ProductionDashboard", {
      onInit: function () {
        ...
        this.setPageTitle(
          this.getView().getModel("i18n").getResourceBundle().getText("pageTitle.production", ["Painel de Produção"])
        );
      },
      ...
      _buildWorkCenterOptions: function () {
        ...
        var aOptions = [
          { key: "ALL", text: this.getView().getModel("i18n").getResourceBundle().getText("workCenter.all") }
        ];
      ...
      onOrderPress: function (oEvent) {
        ...
        var oDlg = new Dialog({
          title: this.getView()
            .getModel("i18n")
            .getResourceBundle()
            .getText("dialog.operations.title", [oData.orderId]),
          contentWidth: "480px",
          contentHeight: "360px",
          ...
          beginButton: new Button({
            text: this.getView().getModel("i18n").getResourceBundle().getText("btn.close"),
            press: function () {
              oDlg.close();
            }
          })
      ...
      formatOrderTitle: function (sOrderId, sMaterial) {
        ...
        return this.getView()
          .getModel("i18n")
          .getResourceBundle()
          .getText("order.title", [sOrderId, sMaterial || "-"]);
      },

      formatNextOp: function (sId, sDesc) {
        if (!sId) {
          return this.getView().getModel("i18n").getResourceBundle().getText("attr.nextOperation.none");
        }
        return sId + ": " + sDesc;
      },
      ...
      formatCriticalityLabel: function (sCrit) {
        var oRB = this.getView().getModel("i18n").getResourceBundle();
        if (sCrit === "High") {
          return oRB.getText("criticality.high");
        }
        if (sCrit === "Medium") {
          return oRB.getText("criticality.medium");
        }
